        ...
        @if(count($items) > 0)
            @foreach($items as $item)
                <article>
                    ...
                    @php $mainImage = $item->images()->where('image_order',0)->first() @endphp
                    <div class="description">
                    @if($mainImage)
                        <a href="{{ URL::to('items') }}/{{ $item->item_id }}" title="{{ $item->title }}"><img src="{{ URL::to('/item_images') }}/{{ $item->item_id }}/{{ $mainImage->filename }}_thumb.{{ $mainImage->extension }}" width="100" style="float:left; margin-right:10px" class="item-image"  /></a>
                    @endif
